                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    eventClick: function(info) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                document.getElementById('update-title').value = response.data
                                    .title;
                                document.getElementById('update-description').value = response
                                    .data.description;
                                document.getElementById('update-start').value = response.data
                                    .start;
                                document.getElementById('update-end').value = response.data.end;
                                document.getElementById('update-color').value = response.data
                                    .color;
                    ...
                    eventDrop: function(info) {
                        if (!confirm("Você solicitou a alteração: " + info.event.title +
                                "\nO evento será alterado para a data: " + moment(info.event.startStr)
                                .format(
                                    'DD-MM-YYYY HH:mm:ss'))) {
                            info.revert();
                        } else {
    ...
    <body>
        <div class="card">
            ...
            <div class="text-start pt-1">
                <p class="text-lg mb-0 text-capitalize" style="font-weight: bold; margin-left: 10px; padding: 5px">Keterangan:
                </p>
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="staticBackdropLabel"
                                {{ auth()->user()->role !== 'admin' ? ' style=display:none;' : '' }}{{ auth()->user()->role !== 'admin' ? 'disabled' : '' }}>
                                Tambah Kegiatan</h5>

                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                                {{ auth()->user()->role !== 'admin' ? ' style=display:none;' : '' }}{{ auth()->user()->role !== 'admin' ? 'disabled' : '' }}></button>
                        </div>
                        <form method="post" action="{{ route('calendar.store') }}"
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="color" name="color" class="form-control form-control-color"
                                        id="create-color" value="#563d7c" title="Choose your color" required>
                                </div>
                                <div class="mb-3">
                                    <label for="datetime-local">Tanggal Mulai</label>
                                    <input type="datetime-local" class="form-control" id="create-start" name="start"
                                        required>
                                </div>
                                <div class="mb-3">
                                    <label for="datetime-local">Tanggal selesai</label>
                                    <input type="datetime-local" class="form-control" id="create-end" name="end"
                                        required>
                                </div>
                            </div>
                            <div class="modal-footer">
                                ...
                                <button type="submit"
                                    class="btn btn-primary"{{ auth()->user()->role !== 'admin' ? ' style=display:none;' : '' }}{{ auth()->user()->role !== 'admin' ? 'disabled' : '' }}>Create</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <form method="POST" action="{{ route('calendar.updateevents') }}">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="exampleColorInput" class="form-label">Warna</label>
                                <input type="color" name="color" class="form-control form-control-color"
                                    id="update-color" title="Choose your color" required
                                    {{ auth()->user()->role !== 'admin' ? 'disabled' : '' }}>
                            </div>
                            <div class="mb-3">
                                <label for="datetime-local">Tanggal mulai</label>
                                <input type="datetime-local" {{ auth()->user()->role !== 'admin' ? 'disabled' : '' }}
                                    class="form-control" id="update-start" name="start" required>
                            </div>
                            <div class="mb-3">
                                <label for="datetime-local">Tanggal selesai</label>
                                <input type="datetime-local" {{ auth()->user()->role !== 'admin' ? 'disabled' : '' }}
                                    class="form-control" id="update-end" name="end" required>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
                            <button type="submit"
                                class="btn btn-primary"{{ auth()->user()->role !== 'admin' ? ' style=display:none;' : '' }}{{ auth()->user()->role !== 'admin' ? 'disabled' : '' }}>Update</button>
                        </div>
                    </form>
                </div>
            </div>
